...
def getOWKey():
    if 'weather' in sys.argv:
        try:
            ...
            print('OpenWeather API key is not present.\nWe can save your key to a configuration.\n')
            userChoice = input("Do you have an OpenWeather API key you would like to provide? ('Yes or No') :")
            if userChoice in ['y','Y','yes','Yes']:
                userOWKey = input("Please enter your OpenWeather API Key:\n")
                keyFile = open('owkey.txt','w+')
                keyFile.write(userOWKey)
                keyFile.close()
            elif userChoice in ['n','N','no','No']:
                print('OpenWeather API Key will not be created.')
                return
    else:
        print("Weather will not be loaded.")
